...
export function writeDeviceEnv(deviceCode: string, config: Partial<DeviceEnvConfig>): void {
  ...
  const lines: string[] = [
    ...
    '',
    '# STREAM GATEWAY (MSE/HLS/WebRTC)',
    `STREAM_GATEWAY_ALWAYS_ON=${config.STREAM_GATEWAY_ALWAYS_ON ?? 'false'}`,
    `STREAM_GATEWAY_AUDIO=${config.STREAM_GATEWAY_AUDIO ?? 'false'}`,
    `SUBSTREAM_URL=${config.SUBSTREAM_URL ?? ''}`,
    '',
    '# PORT FORWARD (nginx stream{} passthrough)',
    `PORTFWD_ENABLED=${config.PORTFWD_ENABLED ?? 'false'}`,
    `PORTFWD_SRC_IP=${config.PORTFWD_SRC_IP ?? ''}`,
    `PORTFWD_SRC_PORT=${config.PORTFWD_SRC_PORT ?? '554'}`,
    `PORTFWD_LISTEN_PORT=${config.PORTFWD_LISTEN_PORT ?? ''}`,
    '',
    '# INFERENCE (Triton)',
